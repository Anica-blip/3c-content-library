/**
 * 3C Content Library - Interactive PDF Viewer
 * Testing viewer with buttons to open JSON in Flipbook or Presentation viewer
 * Version: 2025-01-29 - Cloned from presentation viewer for testing
 */
...
function setupEventListeners() {
    ...
    // Flipbook Viewer button - open current JSON in flipbook viewer
    $('#open-flipbook').on('click', () => {
        const params = getUrlParams();
        if (params.manifest) {
            window.location.href = `flipbook-viewer.html?manifest=${encodeURIComponent(params.manifest)}`;
        } else if (params.content) {
            window.location.href = `flipbook-viewer.html?content=${params.content}`;
        } else {
            alert('No content loaded');
        }
    });
    
    // Presentation Viewer button - open current JSON in presentation viewer
    $('#open-presentation').on('click', () => {
        const params = getUrlParams();
        if (params.manifest) {
            window.location.href = `presentation-viewer.html?manifest=${encodeURIComponent(params.manifest)}`;
        } else if (params.content) {
            window.location.href = `presentation-viewer.html?content=${params.content}`;
        } else {
            alert('No content loaded');
        }
    });
