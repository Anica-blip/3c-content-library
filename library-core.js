...
function openSeriesItem(itemId) {
    ...
    if (item.type === 'presentation') {
        // Device-based routing — same rule as displayContent and library.html,
        // so series presentations also open the correct viewer on phones/tablets.
        const isMobilePresentation = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
            || (navigator.maxTouchPoints > 1 && /Macintosh/i.test(navigator.userAgent));
        const presentationViewerPage = isMobilePresentation ? '../presentation-viewer-mobile.html' : '../presentation-viewer.html';
        const url = item.url ? presentationViewerPage + '?manifest=' + encodeURIComponent(item.url) : presentationViewerPage + '?content=' + item.id;
        window.location.href = url;
        return;
    }
...
function showViewer(content, pdfOnlyMode) {
    ...
    if (content.type === 'pdf') {
        ...
        const isMobilePresentation = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
            || (navigator.maxTouchPoints > 1 && /Macintosh/i.test(navigator.userAgent));
        const presentationViewerPage = isMobilePresentation ? '../presentation-viewer-mobile.html' : '../presentation-viewer.html';
        const presentationUrl = content.url ? presentationViewerPage + '?manifest=' + encodeURIComponent(content.url) : presentationViewerPage + '?content=' + content.id;
