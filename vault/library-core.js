...
function openSeriesItem(itemId) {
    ...
    if (item.type === 'presentation') {
        const isMobile = window.innerWidth <= 768;
        const presentationPage = isMobile ? '../presentation-viewer-mobile.html' : '../presentation-viewer.html';
        const url = item.url ? presentationPage + '?manifest=' + encodeURIComponent(item.url) : presentationPage + '?content=' + item.id;
        window.location.href = url;
        return;
    }
...
function showViewer(content, pdfOnlyMode) {
    ...
    if (content.type === 'pdf') {
        ...
        const isMobilePresentation = window.innerWidth <= 768;
